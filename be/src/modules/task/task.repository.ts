...
import { Task, AuditLog, CreateTaskDTO, TaskStatus, isValidTransition } from './task.types';
...
export class TaskRepository {
  ...
  async updateTaskStatus(
    ...
    try {
      ...
      if (!updatedTask) throw new Error('Failed to retrieve task after update.');
      return updatedTask;
    } catch (err) {
      await connection.rollback();
      throw err;
  ...
  async getTaskAuditLogs(taskId: number, actorUsername: string | null): Promise<AuditLog[]> {
    if (actorUsername) {
