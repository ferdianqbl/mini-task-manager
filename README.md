...
1.  **Administrator Account**:
    - **Username**: `admin`
    - **Password**: `admin123`
    - _Privileges:_ Can view all tasks, view all task audit logs, and access the global system audit log stream.
2.  **Standard User Account**:
    - **Username**: `demo`
    - **Password**: `password123`
    - _Privileges:_ Can only manage and view their own tasks, and view their own log actions ("self logs").
...
4.  **Authentication & Roles**: I chose to implement a production-grade role-based authentication model (stored securely inside HttpOnly cookies) to fulfill the requirements of user ownership, actor validation, and admin log streams in a realistic end-to-end setting.
...
- **State Machine Race Conditions:** If multiple users attempt to update a task's status concurrently, race conditions could lead to invalid sequence jumps or double-logging.
  - _Mitigation:_ Status transitions are locked using a database transaction with a row-level lock (`SELECT ... FOR UPDATE` on the task row), serializing updates.
...
- **AI Assistance:** The AI (Antigravity coding assistant) helped generate initial workspace scaffolding configurations (e.g. `tsconfig.json`, `compose.yml`), standard Tailwind CSS v4 styling rules, and raw SQL queries. The AI assistance is not only generating the code but also helping with the architecture and design patterns. it is also helping with the code review process and identifying potential issues. I used AI for code generation, code completion, and code explanation. I also used AI for code review and identifying potential issues. However, I always double-check the AI-generated code and make sure it is correct and follows the best practices.
- **Validation Methods:**
  - _TypeScript Compilation:_ Running `tsc --noEmit` and build commands to verify type safety.
  - _Manual Testing:_ Triggering invalid status transitions and verifying the backend rejects them with `400 Bad Request`.
  - _Database Integrity Checks:_ Simulating task deletions and verifying the database retained the corresponding audit logs with `task_id = NULL` and the title intact.
